...
let InsertComentarioComponent = class InsertComentarioComponent {
    constructor(dataService, appState, activatedRoute, eventosService) {
...
InsertComentarioComponent.ctorParameters = () => [
...
let ListarComentariosComponent = class ListarComentariosComponent {
    constructor(appState, comentarioService, datosService) {
        this.appState = appState;
    ...
    enlazarEventoYComentario() {
        for (const comentario of this.arrayComentarios) {
            console.log();
            this.datosService.getEventoById(comentario.eventoID).then((evento) => {
                comentario.eventoNombre = evento['nombre'];
                comentario.tipoEvento = evento['lugar'] ? "sitio" : "evento";
            });
            comentario.estrellas = this.convertirNumeroAEstrellas(comentario.valoracion);
        }
    }
...
ListarComentariosComponent.ctorParameters = () => [
...
let ListarEventosComponent = class ListarEventosComponent {
    constructor(eventosService, appState, router, datosService) {
        this.eventosService = eventosService;
        this.appState = appState;
        this.router = router;
        this.datosService = datosService;
    ...
    ordenarPor() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            switch (this.ordenar) {
                ...
                case "SCORE":
                    console.log("score");
                    this.arrayEventos.sort((a, b) => b.nota > a.nota ? 1 : -1);
                    break;
                case "VIEWS":
                    this.arrayEventos.sort(function (a, b) { return b.visitas - a.visitas; });
                    break;
                case "RANDOM":
                    this.arrayEventos = [...this.aleatorizarArray(this.arrayEventos)];
                    break;
            }
            this.actualizarEventosVisibles();
            this.paginaActual = 1;
    ...
    verComentarios(eventoID) {
        ...
        this.datosService.getComentarioByEventoID(eventoID).then((arrayComentarios => {
            ...
            this.app.pagina = "listarComentarios";
        }))
            .catch((err) => { console.log("error al pedir ComentariosByEvento "); });
    }
};
ListarEventosComponent.ctorParameters = () => [
...
let GuardiaGuard = class GuardiaGuard {
...
let MainComponent = class MainComponent {
    constructor(markerService, appStateService) {
...
let BotoneraComponent = class BotoneraComponent {
    constructor(markerService, layerService, datosBack, filtrarService, rutasService, appStateService
    ...
    ngOnInit() {
        ...
        this.datosBack.getBarrios()
            ...
            result['data'].forEach(element => {
                if (!this.distritos.includes(element.distrito_nombre))
                    this.distritos.push(element.distrito_nombre);
            });
        })
    ...
    drop($event) { Object(_angular_cdk_drag_drop__WEBPACK_IMPORTED_MODULE_10__["moveItemInArray"])(this.arrayBotones, $event.previousIndex, $event.currentIndex); }
...
BotoneraComponent.ctorParameters = () => [
...
let MapComponent = class MapComponent {
    constructor(markerService, layerService, datosBack, filtrarService, eventosService, rutasService, appStateService) {
        this.markerService = markerService;
        this.layerService = layerService;
        this.datosBack = datosBack;
        this.filtrarService = filtrarService;
    ...
    updateMyPosition(intervalo = 60000) {
        setInterval(() => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            this.markerService.addMarkerOnMyLocation(this.map, yield this.getPosition());
        }), intervalo);
    }
};
...
let AppStateService = class AppStateService {
    ...
    setPagina(p) { this.app.pagina = p; }
    getPagina() { return this.app.pagina; }
};
...
let datosBackService = class datosBackService {
    constructor(http) {
        this.http = http;
...
datosBackService.ctorParameters = () => [
...
let FiltrarService = class FiltrarService {
    // RENDIMIENTO
...
let LayerService = class LayerService {
    ...
    initBarriosLayer(map, barriosgeoJson) {
        this.map = map;
        this.map.enrutando = false;
        const barrioLayer = leaflet__WEBPACK_IMPORTED_MODULE_2__["geoJSON"](barriosgeoJson, {
            style: (feature) => ({
                weight: 3,
                opacity: 0.6,
                color: '#008f68',
...
let MarkerService = class MarkerService {
    constructor(appStateService, popUpService, eventosService) {
        this.appStateService = appStateService;
    ...
    addMarkerOnPosition(map, position) {
        const marker = leaflet__WEBPACK_IMPORTED_MODULE_2__["marker"](position);
        // console.log(marker)
        marker.addTo(map);
    }
    ...
    addMarkerOnMyLocation(map, position = map.myPosition) {
        const marker = leaflet__WEBPACK_IMPORTED_MODULE_2__["marker"]([position.coords.latitude, position.coords.longitude], { title: "POSICION ACTUAL", draggable: true, icon: this.icono.positionIcon, zIndexOffset: 666666, riseOnHover: true });
        marker.addTo(map);
    ...
    addMarkers(map, arrayDatos, radius = 1000) {
        ...
        for (const [index, elemento] of arrayDatos.entries()) {
            if (elemento.latitude && elemento.longitude) { //comprueba que traen lat y lng antes de añadir
                const marker = leaflet__WEBPACK_IMPORTED_MODULE_2__["marker"]([elemento.latitude + Math.random() / 5000, elemento.longitude + Math.random() / 5000], { icon: this.icono.blueIcon, riseOnHover: true });
                if (map.distance(marker.getLatLng(), this.arrayMarkersMyPosition[this.arrayMarkersMyPosition.length - 1].getLatLng()) < radius) {
                    let pepe = leaflet__WEBPACK_IMPORTED_MODULE_2__["popup"]({ className: "RAJOY" }).setContent("VIVA EL VINO");
...
let PopUpService = class PopUpService {
    constructor() { }
    addPopUp(elemento, special = false) {
        this.texto = "";
...
let LoginComponent = class LoginComponent {
    constructor(router, dataService, appStateService) {
        this.router = router;
        this.dataService = dataService;
        this.appStateService = appStateService;
    }
...
RegistrarComponent.ctorParameters = () => [
    { type: src_app_services_datosBack_service__WEBPACK_IMPORTED_MODULE_2__["datosBackService"] }
];
...
let VacioComponent = class VacioComponent {
    constructor(appState) {
        this.appState = appState;
    }
    ngOnInit() {
